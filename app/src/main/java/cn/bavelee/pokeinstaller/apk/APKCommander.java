...
import moye.installer.ErrorDialog;
...
public class APKCommander {
    ...
    private class InstallApkTask extends Thread {
        ...
        public void run() {
            ...
            final int retCode = ShellUtils.execWithRoot("pm install -r \"" + mApkInfo.getApkFile().getPath() + "\"" + "\n", new ShellUtils.Result() {
                @Override
    ...
    private class ParseApkTask extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                intent.putExtra("content","由于MT以及部分文件管理器调用第三方打开方式时未加一项参数，所以安卓4.4下无法使用这类文件管理器调用“弦 - 安装器”。请更换其他文件浏览器来尝试调用“弦 - 安装器”进行安装！");
